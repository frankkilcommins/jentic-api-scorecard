...
export default function OpidQualityMetadata({ metadata, diagnostics }: OpidQualityMetadataProps) {
  ...
  return (
    <div
      className="mt-3 pt-3 border-t border-[var(--sc-border)] cursor-default space-y-4"
      onClick={(e) => e.stopPropagation()}
    >
      <div className="space-y-1">
        <SectionHeader>Coverage</SectionHeader>
        <div className="flex items-center justify-between text-xs bg-[var(--sc-section)] rounded px-2 py-1.5">
          <span className="text-[var(--sc-text-muted)]">Operations with operationId</span>
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center justify-between text-xs bg-[var(--sc-section)] rounded px-2 py-1.5">
          <span className="text-[var(--sc-text-muted)]">Unique operationIds</span>
          <span className="font-mono font-medium whitespace-nowrap">
            {unambiguous_operation_ids}
            <span className="mx-0.5">/</span>
            {ops_with_operation_id}
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center justify-between text-xs bg-[var(--sc-section)] rounded px-2 py-1.5">
          <span className="text-[var(--sc-text-muted)]">Casing Consistency</span>
          {totalCasingCount > 0 && (
            <span className="font-mono font-medium whitespace-nowrap">
              {consistentCount}
              <span className="mx-0.5">/</span>
      ...
      <div className="border-[var(--sc-border)] border-t pt-3">
        <div className="text-[var(--sc-text-muted)] mb-2 text-[10px] font-medium uppercase tracking-wide">
          Casing Styles
        </div>
        {casingEntries.length > 0 ? (
          ...
          <span className="text-xs text-[var(--sc-text-muted)] italic">None detected</span>
        )}
      </div>

  ...
  return (
    ...
    >
      <div className="flex items-center justify-between text-xs bg-[var(--sc-section)] rounded px-2 py-1.5">
        <span className="text-[var(--sc-text-muted)]">Security Schemes</span>
      ...
      {schemes && schemes.length > 0 ? (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-[var(--sc-text-muted)] text-[10px] font-medium uppercase tracking-wide">
              Configured Schemes
            </span>
            <span className="text-[var(--sc-text-muted)] text-[10px] font-medium uppercase tracking-wide">
              Strength Score
            ...
            {schemes.map((scheme, index) => {
              ...
              return (
                <div key={index} className={`rounded-md px-2 py-1.5 ${colors.bg}`}>
                  ...
                  {details && (
                    <div className="text-[var(--sc-text-muted)] text-[10px] mt-1 ml-6">
                      {details}
                    </div>
                  )}
                  <div className="text-[10px] mt-1 ml-6 italic text-[var(--sc-text-muted)]">
                    {rationale}
                  </div>
                </div>
              );

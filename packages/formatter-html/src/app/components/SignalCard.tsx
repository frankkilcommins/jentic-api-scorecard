...
export default function SignalCard({ signal, diagnostics }: SignalCardProps) {
  ...
  return (
    <div
      className={`border-l-4 bg-[var(--sc-card)] rounded-r-lg p-4 shadow-sm ${isExpandable ? 'cursor-pointer hover:bg-[var(--sc-section)] transition-colors' : ''}`}
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            <h4 className="font-semibold text-[var(--sc-text-primary)]">{signal.name}</h4>
            {isExpandable && (
              <svg
                className={`w-5 h-5 text-[var(--sc-text-muted)] transition-transform ${expanded ? 'rotate-180' : ''}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <p className="text-sm text-[var(--sc-text-secondary)]">{signal.description}</p>
